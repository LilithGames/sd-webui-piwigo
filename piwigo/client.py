...
class Ws:
    ...
    async def async_call(self, **kw):
        ...
        p = None
        if 'image' in kw:
            if type(kw['image']) == str:
                p = await aiofiles.open(kw['image'], mode='rb')
                data = await p.read()
                files = {'image': (kw['image'], data)}
            else:
                files = {'image': ('image', kw['image'])}
            params = { i : params[i] for i in params if i != 'image'}
        else:
            files = {}
        if self._post_only:
            ...
            async with httpx.AsyncClient() as client:
                r = await client.post(url, params=params, data=data, files=files, cookies=self._pwg._cookies)  
        else:
            async with httpx.AsyncClient() as client:
                r = await client.get(url, params=params, cookies=self._pwg._cookies)  
        if p : await p.close()
        try:
            result = r.json()
            if result['stat'] == 'fail':
            ...
            raise WsErrorException(f"url: {url}, body: {r.text}")
